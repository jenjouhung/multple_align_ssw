...
class Aligner(object):
    ...
    def align(self, query='', reference=None,qMode= False, revcomp=True):
        ...
        # qMode : 是否啟用 Joey Quick Mode, 一旦使用 Joey Quick Mode, flag的設定可能就沒有用途。
        ...
        #呼叫底層_align函式進行比較
        res = self._align(query, reference, flags, filter_score, filter_distance, mask_length,qMode)
        #若上層關掉此選項，之後不會進去。
        if revcomp:
    ...
    #
    def _align(self, query, reference, flags, filter_score, filter_distance, mask_length, qMode,score_size=2): 
        ...
        if (not qMode): 
            ...
            alignment = libssw.ssw_quick_align_init(
                profile, #ssw_profile_p
                _reference, #POINTER(c_int16) #2020/02/23 修正
                len(_reference), #c_int32
                self.gap_open, #c_uint8
...
class Alignment(object):
    def __init__ (self, alignment, query, reference, matrix=None, encoding="latin"):
        ...
        # ADD 2020.03.13
        self.refLen = alignment.contents.refLen
        self.readLen = alignment.contents.readLen
        # self.reference_begin2 = alignment.contents.ref_begin2
        # self.query_begin2 = alignment.contents.read_begin2
        # END
        ...
        # self._cigar_string_n = [alignment.contents.cigar_n[idx] for idx in range(alignment.contents.cigarLen_n)]
        ...
        #    print("{} ".format(query[i]),end="")
        # print("")

        # for i in range(self.refLen):
        #     print("{} ".format(reference[i]),end="")
        #     for j in range(self.readLen):
        #         # # 計算第j值，需要讀取哪個位置
        #         readpos = i*self.readLen+j
        #         print("[{},{}]{}({}) ".format(
        #             i,j,
        #             alignment.contents.scoreArray[readpos],
        #             alignment.contents.moveArray[readpos],
        #             ),end="")                                      
        #     print("")      
    ...
    # @property
    # def iter_cigar_n(self):
    #     for val in self._cigar_string_n:
    #         op_len = libssw.cigar_int_to_len(val)
    #         op_char = libssw.cigar_int_to_op(val).decode(self._encoding)
    #         yield (op_len, op_char)
    ...
    @property
    def alignment(self):
       return  self.cigar_to_message(
           self.iter_cigar,
           self.reference_begin,self.reference_end,
           self.query_begin,self.query_end)
 
    # @property
    # def alignment_n(self):
    #    return  self.cigar_to_message(
    #        self.iter_cigar_n,
    #        self.reference_begin2,self.reference_end,
    #        self.query_begin2,self.query_end)
    ...
    def cigar_to_message(self,cigar_iter,reference_begin,reference_end,query_begin,query_end):
        r_index = 0
        q_index = 0
        r_seq = self.reference[reference_begin: reference_end + 1]
        q_seq = self.query[query_begin: query_end + 1]
        ...
        for (op_len, op_char) in cigar_iter:
            # #列印 cigar 細節
            # print("[op={}][op_len={}]".format(op_char,op_len))
            op_len = int(op_len)
            if op_char.upper() == 'M':
